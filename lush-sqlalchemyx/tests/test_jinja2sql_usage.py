...
def _async_j2sql() -> Jinja2SQL:
    return Jinja2SQL(jinja2.Environment(enable_async=True, autoescape=True))


def _file_j2sql(path: Path, *, async_env: bool = False) -> Jinja2SQL:
    env = jinja2.Environment(
        loader=jinja2.FileSystemLoader(path),
        enable_async=async_env,
        autoescape=True,
    )
...
def test_custom_filter_array_literal() -> None:
    ...
    def array_filter(self: Jinja2SQL, value: list[str]) -> str:
        return identifier(self, ", ".join(f"'{item}'" for item in value))

    j2sql.register_filter("array2", array_filter, bind=True)
...
async def test_from_file_async_named(tmp_path: Path) -> None:
    ...
    j2sql = _file_j2sql(tmp_path, async_env=True)
...
def test_register_filter_method() -> None:
    ...
    def csv(self: Jinja2SQL, value: list[str]) -> str:
        return ", ".join(identifier(self, f"'{v}'") for v in value)

    j2sql.register_filter("csv", csv, bind=True)
