...
class AbstractSyncWriteDAL(ABC, Generic[SessionT, EntityT, DTOModelT, CUModelT, PrimaryKeyT]):
    """同步写入 DAL 抽象基类.
    ...
    @classmethod
    @abstractmethod
    def update_only_set_by_id(cls, session: SessionT, entity_id: PrimaryKeyT, cu: CUModelT, need_refresh: bool = False) -> EntityT | None:
    ...
    @classmethod
    @abstractmethod
    def delete_by_id(cls, session: SessionT, entity_id: PrimaryKeyT) -> bool:
        """根据主键 ID 删除实体 (软删除或物理删除)."""
        ...


class AbstractAsyncWriteDAL(ABC, Generic[SessionT, EntityT, DTOModelT, CUModelT, PrimaryKeyT]):
    """异步写入 DAL 抽象基类.
    ...
    async def update_only_set_by_id(
        cls, session: SessionT, entity_id: PrimaryKeyT, cu: CUModelT, need_refresh: bool = False
    ) -> EntityT | None:
        """仅更新 CU 中已设置 (非 unset) 的字段."""
        ...
    ...
    @classmethod
    @abstractmethod
    async def delete_by_id(cls, session: SessionT, entity_id: PrimaryKeyT) -> bool:
        """根据主键 ID 删除实体 (软删除或物理删除)."""
        ...
